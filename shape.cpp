...
Line::Line(QPaintDevice *device, int id)
    : Shape(device, id, ShapeType::Line), line_begin(1,1), line_end(1,1)
{}
...
double Polygon::perimeter()
{
    ...
    for(int i = 0; i < vertVect.size() - 1; i++)
    {
...
void Polygon::move(const int x, const int y, int vertex)
{
    ...
    if(vertex <= numVerts)
    {
       if((vertex - 1) < vertVect.capacity())
       {
           vertVect[vertex] = temp;
       }
...
vector<QPoint> & Polygon::getVertices()
{
    return vertVect;
}
